...
- Panel widths automatically rebalance when optional panels are hidden
...
By default, RadarWise centers the radar on the latitude and longitude configured in Home Assistant. Set `latitude` and `longitude` only when a card should use a different radar center.
...
| `card_height` | No |  | Optional fixed card height in pixels, 180-1200. Useful for panel dashboards with extra vertical room. |
